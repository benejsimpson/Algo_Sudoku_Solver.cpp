...
struct Cell {
    ...
    void place_Z_digit(int digit) { Z_digit = digit ; C_digits = 0; }
    ...
    int count_C_digits() { bit9 candidates = C_digits; return candidates.count(); }
};
...
struct Solver {
    ...
    */
    ...
    // fills all naked singled in grid
    // returns number of digits filled
    int naked_single();
    // fills hidden singles in a constraint
    // returns num singles filled
    int hidden_single_in_constraint(Constraint &con);
    // fills hidden singles in every constraint
    // returns number of digits filled
    int hidden_single_fill();
    // find dominos in constraint
    int find_dominos_in_constraint(Constraint &con);
    // pass in unsolved constraints
    int find_all_dominos();
    bool check_solved();
};
...
bool SudokuSolver(Solver &s){
    ...
    do {
        ...
        filled += s.find_all_dominos(); // remove candidates seeing dominos
        if (filled == 0) failed++;
...
void SetupStartingBoard(Solver &s, string BoardHash) {
    ...
    for (char c : BoardHash) {
        if (c == '.') {pos +=3; continue;}
        digit = c - '0';
        if (digit != 0) { s.place_Z_digit(pos,digit); }
        pos++;
    }
}
...
void Cell::__cell__(int cell_pos) {
    pos = cell_pos;
...
int Solver::naked_single(){
    ...
    do {
        ...
        for (int pos = 0; pos < 81; pos++) {if ( board[pos].count_C_digits() == 1 ){
...
int Solver::hidden_single_in_constraint(Constraint &con){
    ...
    */
    ...
    if (con.solved_constraint) return 0;
    int filled  = 0;

    for (int digit = 1; digit <= 9; digit++) {
        if (!(missing & bit_for_digit(digit))) continue;
        int count = 0;
        int last_pos = -1;
        for (int i = 0; i < 9; i++) {
            int pos = con.cells[i];
...
bool Solver::check_solved() {
        ...
        int solved = 0;
        for (int i = 0; i < 9; i++) solved += rows[i].solved_constraint;
        return solved == 9;
    }

int Solver::find_dominos_in_constraint(Constraint &con) {
    if (con.solved_constraint) return 0; // if constraint is solved -> continue
    int changes = 0;
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if ( board[pos0].is_solved() || board[pos0].count_C_digits() != 2 ) continue;
        ...
        for (int j = i+1; j < 9; j++) {
            ...
            if ( (pos0 == pos1) || (board[pos1].is_solved()) ) continue; 

            BitDigit cdig1 = board[pos1].C_digits; // bitdig of second cell

            if (cdig0 == cdig1){ // both cells have same vals
                vector <int> digs = get_set_bits(cdig0);
                // remove digits from other cells in shared constraints
                if (board[pos0].row_index == board[pos1].row_index) changes += remove_C_dominos(digs, pos0,pos1,rows[board[pos0].row_index]);
                if (board[pos0].col_index == board[pos1].col_index) changes += remove_C_dominos(digs, pos0,pos1,cols[board[pos0].col_index]);
                if (board[pos0].box_index == board[pos1].box_index) changes += remove_C_dominos(digs, pos0,pos1,boxes[board[pos0].box_index]);
            }
        }
    }
    return changes;
...
int Solver::remove_C_dominos(vector<int> digs, int pos1, int pos2, Constraint &con) {
    ...
    for (int i = 0; i < 9; i++){
        int pos = con.cells[i];
        if ((board[pos].is_solved()) || (pos == pos1) || (pos == pos2)) continue;
        for (int d : digs) {
...
int Solver::find_all_dominos() {
    ...
    for (int i = 0; i < 9; i++){
        ...
        filled += find_dominos_in_constraint(rows[i]);
        filled += find_dominos_in_constraint(cols[i]);
        filled += find_dominos_in_constraint(boxes[i]);
    }
        total_filled += filled;
    return filled;
}